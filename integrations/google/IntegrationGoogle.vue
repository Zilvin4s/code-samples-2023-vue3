                    ...
                    <div class="col">
                      <div>
                        ...
                        <div v-if="status.connection === true" class="fs-6 text-muted mt-1">
                          {{ t('All your active calendars sync without issues.') }}
                        </div>
                        <div
...
import {
  ...
  checkCalendars,
  toogleCalendar
} from '@/api/Business/integrations/integrationsGoogle'
...
const hasLocations = computed(() => {
  return false // TODO with backend
})
...
function testConnection() {
  ...
  checkConnection(props.integration.id)
    ...
    .catch((err) => {
      ...
      serverErrors.value.connection =
        err?.response?.data?.message || err?.message || t('Error with connection test')
    })
    .finally(() => {
...
async function init() {
  ...
  Promise.all([
    resetErrors(),
    getIntegrationInfo().catch(handleIntegrationError),
    getCalendarsData().catch(handleCalendarsError)
  ]).finally(() => {
    busy.value.loading = false
  })
}

function handleCalendarsError(e: any) {
  serverErrors.value.critical =
    e?.response?.data?.message || e?.message || 'Unknown error with calendars'
}
